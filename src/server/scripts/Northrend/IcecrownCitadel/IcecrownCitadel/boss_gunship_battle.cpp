 ...
 *
 * Copyright (C) 2010 - 2012 Myth Project <http://bitbucket.org/the_sun/myth-core/>
 *
...
//Teleport players
void TeleportPlayers(Map* map, uint32 mapid, uint64 TeamInInstance)
{
    if(map)
    {
            ...
            for(Map::PlayerList::const_iterator itr = lPlayers.begin(); itr != lPlayers.end(); ++itr)
            {
                if(Player* pPlayer = itr->getSource())
                {
                    ...
                    if(TeamInInstance == ALLIANCE)
                        pPlayer->TeleportTo(mapid, -437.498505f, 2425.954f, 192.997f, 2.247f);
                    else
                        pPlayer->TeleportTo(mapid, -437.498505f, 1997.954f, 192.997f, 2.247f);
                }
            }
        }
    }
...
void StopFight(Transport* t1, Transport* t2)
{
    ...
    for(Transport::CreatureSet::iterator itr = t1->m_NPCPassengerSet.begin(); itr != t1->m_NPCPassengerSet.end();)
    {
        if(Creature *npc = *itr)
        {
            ...
            && npc->GetEntry() != NPC_GB_MURADIN_BRONZEBEARD)
            {
...
class npc_muradin_gunship : public CreatureScript
{
    public:
        ...
        struct npc_muradin_gunshipAI : public ScriptedAI
        {
            ...
            {
                if(_instance->GetData(DATA_TEAM_IN_INSTANCE) == HORDE && me->GetHealthPct() < 1.0f )
                {
                   damage = 0;
                   if(Creature* pSaurfang = ObjectAccessor::GetCreature(*me, me->GetInstanceScript()->GetData64(DATA_GB_HIGH_OVERLORD_SAURFANG)))
                       pSaurfang->AI()->DoAction(ACTION_DONE);
                }

                if(_instance->GetData(DATA_TEAM_IN_INSTANCE) == ALLIANCE && me->GetHealthPct() < 1.0f )
                {
                   damage = 0;
                   me->AI()->DoAction(ACTION_FAIL);
                }
                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    switch(eventId)
                    {
                        ...
                        case EVENT_OUTRO_ALLIANCE_2:
                            StopFlyShip(skybreaker);
                            me->SummonGameObject(RAID_MODE(GO_CAPITAN_CHEST_A_10N, GO_CAPITAN_CHEST_A_25N, GO_CAPITAN_CHEST_A_10H, GO_CAPITAN_CHEST_A_25H), -426.0f,  2412.1f, 191.1f, 0, 0, 0, 0, 0, 100000);
                        ...
                        case EVENT_OUTRO_ALLIANCE_3:
                            ...
                            break;
                        case EVENT_FAIL:
                            TeleportPlayers(map, me->GetMapId(), ALLIANCE);
    ...
    public:
        ...
        struct npc_sergeantAI : public ScriptedAI
        {
            ...
            void Reset()
            {
                ...
                me->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_DISABLE_MOVE);
                me->SetReactState(REACT_PASSIVE);
                events.ScheduleEvent(EVENT_WALK_MOBS, 3000);
                desperated = false;
                me->RemoveAurasDueToSpell(SPELL_EXPERIENCED);
                me->RemoveAurasDueToSpell(SPELL_ELITE);
                DesperateResolve = RAID_MODE( SPELL_DESPERATE_RESOLVE_10_NM, SPELL_DESPERATE_RESOLVE_25_NM, SPELL_DESPERATE_RESOLVE_10_HM, SPELL_DESPERATE_RESOLVE_25_HM);
    ...
    public:
        ...
        struct npc_gunship_mageAI : public Scripted_NoMovementAI
        {
            ...
            void UpdateAI(const uint32 diff)
            {
                ...
                } else DoCast(me,SPELL_SHADOW_CHANNELING);
            }
            ...
            void UpdateAI(const uint32 diff)
            {
                if(me->HasAura(SPELL_BELOW_ZERO))
                {
                    ...
                    if(Vehicle* veh = me->GetVehicleKit())
                        veh->RemoveAllPassengers();
                } else me->RemoveFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NOT_SELECTABLE);
            }
                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    switch(eventId)
                    {
                        ...
                        case EVENT_OUTRO_HORDE_2:
                            StopFlyShip(orgrimmar);
                            me->SummonGameObject(RAID_MODE(GO_CAPITAN_CHEST_H_10N, GO_CAPITAN_CHEST_H_25N, GO_CAPITAN_CHEST_H_10H, GO_CAPITAN_CHEST_H_25H), -426.0f,  2412.1f, 191.1f, 0, 0, 0, 0, 0, 100000);
                            me->AddUnitMovementFlag(MOVEMENTFLAG_WALKING);
                            me->GetMotionMaster()->MovePoint(0, -590.700f, 2213.01f, 539.1f);
                            break;
                        case EVENT_OUTRO_HORDE_3:
                            ...
                            break;
                        case EVENT_FAIL:
                            TeleportPlayers(map, me->GetMapId(), HORDE);
...
class spell_gb_burning_pitch : public SpellScriptLoader
{
    public:
        ...
        class spell_gb_burning_pitch_SpellScript : public SpellScript
        {
            ...
            void Register()
            {
                OnEffect += SpellEffectFn(spell_gb_burning_pitch_SpellScript::HandleDummy, EFFECT_0, SPELL_EFFECT_DUMMY);
            }
        };
