...
void World::LoadConfigSettings(bool reload)
{
    ...
    if(reload)
    {
        uint32 val = World::GetRealmPort();
        if(val != m_int_configs[CONFIG_PORT_WORLD])
            sLog->outError("World Server Port option can't be changed at reload, using current value (%u).", m_int_configs[CONFIG_PORT_WORLD]);
    }
    else
        m_int_configs[CONFIG_PORT_WORLD] = World::GetRealmPort();
...
void World::SetInitialWorldSettings()
{
    ...
    sLog->outString();
    sLog->outString("Loading Myth strings...");
...
void World::DetectDBCLang()
{
    ...
    sLog->outString("DBC Locale: %s", localeNames[m_defaultDbcLocale]);
    sLog->outString();
}
...
/// Send text to players selected by account access
void World::SendTextByAccess(AccountTypes security, bool not_less, int32 string_id, ...)
{
    va_list ap;
    ...
    for (SessionMap::const_iterator itr = m_sessions.begin(); itr != m_sessions.end(); ++itr)
    {
        ...
        if ((itr->second->GetSecurity() < security) == not_less)
            continue;
...
uint32 World::GetRealmPort()
{
    QueryResult result = LoginDatabase.PQuery("SELECT `port` FROM `realmlist` WHERE `id` = '%u'", realmID);
