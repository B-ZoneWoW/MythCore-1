    ...
    switch(spellId)
    {
        ...
        case 58730: // No fly Zone - Wintergrasp
            if(sWorld->getBoolConfig(CONFIG_OUTDOORPVP_WINTERGRASP_ENABLED))
            {
                ...
                if(!pvpWG->isWarTime() || player->isInFlight())
                    return false;
            }
            break;
        ...
        case 57940: // Essence of Wintergrasp - Northrend
            if(!player || !pvpWG)
                return false;
            if((uint8)player->GetTeamId() != sWorld->getWorldState(WORLDSTATE_WINTERGRASP_CONTROLING_FACTION))
                return false;
            break;
        case 55774: // Horde Control Phase
            if(newZone == 4197)
                if(sWorld->getWorldState(WORLDSTATE_WINTERGRASP_DEFENDERS) == TEAM_ALLIANCE)
                     return false;
            break;
        case 55773: // Alliance Control Phase
            if(newZone == 4197)
                if(!sWorld->getWorldState(WORLDSTATE_WINTERGRASP_DEFENDERS) == TEAM_ALLIANCE)
                    return false;
            break;
    ...
    for(uint32 i = 0; i < sSpellStore.GetNumRows(); ++i)
    {
        ...
        switch(i)
        {
            ...
            case 48278: // Sword Ritual (Utgarde - Slava)
                ...
                break;
            case 64415:
                spellInfo->EffectTriggerSpell[1] = 64411;
                ++count;
                break;
