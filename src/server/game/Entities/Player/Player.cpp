...
inline void KillRewarder::_RewardHonor(Player* pPlayer)
{
    // Rewarded player must be alive.
    if(pPlayer->isAlive())
        pPlayer->RewardHonor(_victim, _count, -1, true);
    //if(pPlayer->GetZoneId() == 210)
    //    if(pPlayer->hasQuest(13234))
    //        pPlayer->
}
...
void Player::HandleDrowning(uint32 time_diff)
{
    ...
    if(m_MirrorTimerFlags & UNDERWATER_INLAVA && !(m_lastLiquid && m_lastLiquid->SpellId))
    {
        // Breath timer not activated - activate it
...
void Player::Update(uint32 p_time)
{
    ...
    if(isCharmed()) {
        if(Unit* pCharmer = GetCharmer()) {
            if(pCharmer->GetTypeId() == TYPEID_UNIT && pCharmer->isAlive())
                UpdateCharmedAI();
        }
    }
    ...
    if(!TimedAchievements.empty())
    {
        std::list<AchievementTimedData>::iterator EraseIterator = TimedAchievements.end();
        for(std::list<AchievementTimedData>::iterator itr = TimedAchievements.begin(); itr != TimedAchievements.end(); ++itr)
        {
            (itr)->TimeNow += p_time;
            if((itr)->IsCompleted() || (itr)->IsFailed())
                EraseIterator = itr;
        }
        if(EraseIterator != TimedAchievements.end())
            TimedAchievements.erase(EraseIterator);
    }
    ...
    if(m_deathTimer > 0 && !GetBaseMap()->Instanceable() && getDeathState() != GHOULED)
    {
    ...
    Pet* pPet = GetPet();
    if(pPet && !pPet->IsWithinDistInMap(this, GetMap()->GetVisibilityRange()) && !pPet->isPossessed())
    //if(pet && !pet->IsWithinDistInMap(this, GetMap()->GetVisibilityDistance()) && (GetCharmGUID() && (pet->GetGUID() != GetCharmGUID())))
        RemovePet(pPet, PET_SAVE_NOT_IN_SLOT, true);
...
bool Player::TeleportTo(uint32 mapid, float x, float y, float z, float orientation, uint32 options)
{
    ...
    if((GetSession()->GetSecurity() < SEC_GAMEMASTER_LOW) && sDisableMgr->IsDisabledFor(DISABLE_TYPE_MAP, mapid, this))
    {
...
void Player::SetGameMaster(bool on)
{
    ...
    else
    {
        ...
        RemoveFlag(PLAYER_FLAGS, PLAYER_FLAGS_GM | PLAYER_FLAGS_DEVELOPER);
...
void Player::GiveXP(uint32 xp, Unit* victim, float group_rate)
{
    ...
    // 571 Northrend, 530 Outland, 16 Azshara, 28 & 139 Plaguelands, 1377 Silithus
    if((level < 66 && GetMapId() == 571) || (level < 43 && zone == 16) || (level < 49 && (zone == 28 || zone == 139 || zone == 1377)))
        return;
    ...
    switch(gossipOptionId)
    {
        ...
        case GOSSIP_OPTION_LEARNDUALSPEC:
            if(GetSpecsCount() == 1 && getLevel() >= sWorld->getIntConfig(CONFIG_MIN_DUALSPEC_LEVEL))
            {
                ...
                CastSpell(this, 63645, true, NULL, NULL, GetGUID());
                PlayerTalkClass->SendCloseGossip();
            }
            break;
        case GOSSIP_OPTION_UNLEARNTALENTS:
...
void Player::_SaveQuestStatus(SQLTransaction& trans)
{
    ...
    for(saveItr = m_QuestStatusSave.begin(); saveItr != m_QuestStatusSave.end(); ++saveItr)
    {
        if(saveItr->second)
        {
            ...
            {
                trans->PAppend("REPLACE INTO character_queststatus (guid, quest, status, explored, timer, mobcount1, mobcount2, mobcount3, mobcount4, itemcount1, itemcount2, itemcount3, itemcount4, itemcount5, itemcount6) "
                    "VALUES ('%u', '%u', '%u', '%u', '" UI64FMTD "', '%u', '%u', '%u', '%u', '%u', '%u', '%u', '%u', '%u', '%u')",
                    GetGUIDLow(), statusItr->first, statusItr->second.m_status, statusItr->second.m_explored, uint64(statusItr->second.m_timer / IN_MILLISECONDS+ sWorld->GetGameTime()), statusItr->second.m_creatureOrGOcount[0], statusItr->second.m_creatureOrGOcount[1], statusItr->second.m_creatureOrGOcount[2], statusItr->second.m_creatureOrGOcount[3], statusItr->second.m_itemcount[0], statusItr->second.m_itemcount[1], statusItr->second.m_itemcount[2], statusItr->second.m_itemcount[3], statusItr->second.m_itemcount[4], statusItr->second.m_itemcount[5]);
            }
        }
        else
...
{
    AuraEffect const* pAura = m_runes->runes[index].ConvertAura;
    ConvertRune(index, GetBaseRune(index));
    SetRuneConvertAura(index, NULL);

    // Don't drop passive talents providing rune convertion
    if(!pAura || pAura->GetAuraType() != SPELL_AURA_CONVERT_RUNE)
        return;

    for(int i = 0; i < MAX_RUNES; ++i) {
        if(pAura == m_runes->runes[i].ConvertAura)
            return;
    }

    pAura->GetBase()->Remove();
}
...
void Player::ActivateSpec(uint8 spec)
{
    ...
    UnsummonAllTotems();
    RemoveAllControlled();
    RemoveAura(28682); // Mage: Combustion
    RemoveAura(46619); // DK: Rise Ally
    RemoveAura(31223); // Rogue: Master of Subtlety
    RemoveAurasByType(SPELL_AURA_MOD_SPELL_CRIT_CHANCE);
    if(GetPet())
...
{
    AchievementTimedData atd(AchievementId,time,maxprogress);
    TimedAchievements.push_back(atd);
}

void Player::UpdateKillingTimedAchievementProgress(uint32 entry, uint8 type)
{
    uint32 AchievementId = 0;
    uint32 InitialAchievementTime = 0;
    uint32 RequiredProgress = 999;
    uint32 ProgressCount = 1;
    ...
    switch(type)
    {
        ...
        case TYPEID_UNIT:
        {
            switch(entry)
            {
                // This Cases = NPC entry
                case 23801:  // All Turkey npc
                case 24746:
                case 32818:
                case 32820:
                case 32905:
                case 32956:
                    AchievementId = 1254;
                    InitialAchievementTime = 180000; // Timer
                    RequiredProgress = 15; // KILL count
                    break;
                case 10161:  //Leeeeeroy
                    AchievementId = 2188;
                    InitialAchievementTime = 15000;
                    RequiredProgress = 50;
                    break;
            }
        }
    ...
    if(AchievementId)
    {
        AchievementTimedData* atd = GetTimedAchievement(AchievementId);
        if(atd)
        {
            UpdateTimedAchievementProgress(AchievementId,ProgressCount);
            if(atd->IsCompleted())
                GetAchievementMgr().CompletedAchievement(sAchievementStore.LookupEntry(AchievementId));
        } else {
            SetTimedAchievement(AchievementId,InitialAchievementTime,RequiredProgress);
            UpdateTimedAchievementProgress(AchievementId,ProgressCount);
            atd = GetTimedAchievement(AchievementId);
        }
    }
}
